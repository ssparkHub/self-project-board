...
class PaginationServiceTest {
    ...
    private final PaginationService sut; //스프링부트 테스트가 아니라면 new생성자를 통해 만든다

    PaginationServiceTest(@Autowired PaginationService paginationService) {
        this.sut = paginationService;
    }

    @DisplayName("현재페이지 번호와 총 페이지 수를 주면, 페이징 바 리스트를 만들어 준다")
    @MethodSource
    @ParameterizedTest(name = "[{index}] 현재페이지: {0}, 총페이지: {1} => {2}") //"[{현재페이지}] {입력값} => {출력값}" -- j5유닛
